...
class LineaSaml2Backend(Saml2Backend):
    def authenticate(
        ...
        **kwargs,
        ...
        # session_info carrega a assertion completa (uid, email, grupos) — PII:
        # só em DEBUG (SAML_DEBUG), nunca em produção
        logger.debug(f"session_info: {session_info}")
        logger.debug(f"attribute_mapping: {attribute_mapping}")
        logger.debug(f"create_unknown_user: {create_unknown_user}")
        ...
        logger.info(f"idp_name: {idp_name}")  # identificador do IdP, não PII
    ...
    def _extract_user_identifier_params(
        ...
        try:
            ...
            logger.debug(f"User Lookup Key: {user_lookup_key} Value: {user_lookup_value}")
            if user_lookup_value in [None, "None", ""]:
    ...
    def save_user(self, user, *args, **kwargs):
        logger.debug(f"save_user() user: {user}")
    ...
    def setup_groups(self, user, attributes: dict):
        ...
        logger.debug(f"User has been added to the following groups: {groups}")
